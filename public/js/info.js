document.addEventListener("DOMContentLoaded", async function () {
    ...
    try {
        ...
        if (data.sucesso) {
            ...
            nota.textContent = jogo.nr_nota;
            atualizarCorNota(nota, jogo.nr_nota); // Atualizar cor da nota
    ...
    notaSlider.addEventListener("input", () => {
        const nota = parseFloat(notaSlider.value);
        notaDinamica.textContent = nota;
        atualizarCorNota(notaDinamica, nota);
    });
    ...
    salvarNota.addEventListener("click", async function () {
        ...
        const listaSelecionada = document.getElementById("listasSelect").options[document.getElementById("listasSelect").selectedIndex].text;
        ...
        // Verificar se a lista selecionada é "Desejo"
        if (listaSelecionada === "Desejo") {
            alert("Você não pode dar uma nota para um jogo na lista 'Desejo'.");
            await adicionarJogoALista(idLista, idUsuario, gameId, token);
...
async function adicionarJogoALista(idLista, idUsuario, gameId, token) {
    try {
...
function atualizarCorNota(elemento, nota) {
    const hue = (120 * nota) / 10; // 0 -> vermelho, 10 -> verde
